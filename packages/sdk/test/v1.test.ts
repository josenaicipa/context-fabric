...
const chunks: ContextChunk[] = [
  {
    id: "c1",
    text: "Acme support onboarding",
    project: "acme-shop",
    channel: "#support",
    sensitivity: "public",
    source: { sourceId: "doc-1", title: "Docs" },
    score: 2,
  },
  {
    id: "c2",
    text: "Other private note",
    project: "other-co",
    channel: "#support",
...
test("v1 bundle includes citations and contamination drops", () => {
  const bundle = new Fabric({
    budgetProfiles: { general: { name: "general", maxTokens: 2000 } },
  }).assemble(
    { query: "onboard", project: "acme-shop", channel: "#support", maxSensitivity: "public" },
    chunks,
  );
  assert.deepEqual(
...
test("pack, handoff and eval helpers are public-demo usable", () => {
  ...
  const bundle = new Fabric().assemble(
    { query: "onboard", project: "acme-shop", channel: "#support" },
    chunks,
  );
  assert.match(renderAgentContext(bundle), /AGENT_CONTEXT/);
  const report = runEvals(new Fabric(), [
    {
      name: "demo",
      request: bundle.request,
      chunks,
      expectedChunkIds: ["c1"],
      forbiddenChunkIds: ["c2"],
    },
  ]);
...
test("v1 drop paths are observable", () => {
  ...
  const bundle = new Fabric({ budget: { maxTokens: 20 } }).assemble(
    { query: "q", project: "acme-shop", channel: "#support", maxSensitivity: "public" },
    [
      {
        id: "restricted",
        text: "private",
