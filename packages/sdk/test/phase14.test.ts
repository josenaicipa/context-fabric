...
const memoryRecords: MemoryRecord[] = [
  ...
  { id: "m3", summary: "Other", content: "Wrong project", project: "other-co", channel: "#support", status: "verified" },
];
...
test("runPreflight produces agent context and excludes candidates by default", () => {
  ...
  assert.deepEqual(result.chunks.map((chunk) => chunk.id), ["memory:m1", "repo:guide"]);
...
test("runPreflight can explicitly include candidates", () => {
  ...
  assert.deepEqual(result.chunks.map((chunk) => chunk.id), ["memory:m1", "memory:m2", "repo:guide"]);
});
