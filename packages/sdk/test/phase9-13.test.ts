...
import {
  Fabric,
  createAssemblePayload,
  createDebugHtmlPayload,
...
test("memoryRecordsToChunks excludes candidates and cross-project records by default", () => {
  ...
  const bundle = new Fabric().assemble(
    { query: "handoff", project: "acme-shop", channel: "#support", maxSensitivity: "internal" },
    chunks,
  );
...
test("API payload helpers produce server-compatible request body", () => {
  ...
  const chunks = memoryRecordsToChunks(memories, {
    project: "acme-shop",
    channel: "#support",
