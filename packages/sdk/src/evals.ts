...
export function runEvals(fabric: Fabric, cases: EvalCase[]): EvalReport {
  let hits = 0, expected = 0, contaminated = 0, forbidden = 0, leaks = 0;
  for (const item of cases) {
    ...
    leaks += bundle.chunks.filter((chunk) => RESIDUAL_SECRET_PATTERNS.some((pattern) => pattern.test(chunk.text))).length;
  }
  const recall = expected ? hits / expected : 1;
  const contamination = forbidden ? contaminated / forbidden : 0;
  return { cases: cases.length, recall, contamination, secretLeaks: leaks, passed: recall >= 0.9 && contamination === 0 && leaks === 0 };
}
